...
contract yCORE is Context, IERC20 {
    ...
    address internal _STAKERADDRESS;

    constructor (address _stakerAddress, string memory name, string memory symbol) public {
        _STAKERADDRESS = _stakerAddress;
        _name = name;
        _symbol = symbol;
        _decimals = 18;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        // ***** Burn 5% on each transfer if not from/to uniswap
        
        if (sender != UNIPOOL && recipient != UNIPOOL && sender != UNIROUTER && recipient != UNIROUTER) {
            uint256 burned = amount.div(20); 
            tokensToTransfer = amount.sub(burned);
    ...
    function createUniswap() public payable{
        ...
        UNIPOOL = Uniswap(FACTORY).getPair(address(this), WETHAddress);
    }
